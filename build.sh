...
debug_compile() {
    ...
    log_section "Installing host tools"
    make -j$(nproc) tools/install V=s || { echo -e "${RED}Failed to install tools${NC}"; exit 1; }
...
# This prevents the build from trying to compile LLVM from source (saving hours)
log_section "Extracting LLVM prebuilt (Qualcommax Specific)"
if [ ! -d "llvm-bpf" ]; then
    echo -e "${YELLOW}Downloading LLVM BPF for IPQ807x...${NC}"
    wget -q https://downloads.openwrt.org/snapshots/targets/qualcommax/ipq807x/llvm-bpf-21.1.6.Linux-x86_64.tar.zst
    tar --zstd -xvf llvm-bpf-21.1.6.Linux-x86_64.tar.zst
else
    echo -e "${GREEN}LLVM BPF already present. Skipping download.${NC}"
fi
